#!/usr/bin/env bash
set -euo pipefail

# 2 nodes × 4 GPUs = 8 processes total. If you use a single node with 4× A4000: use
# --num_machines=1 --num_processes=4 and run only this script (no node1).

# Repo root (parent of configs/)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
cd "${ROOT_DIR}"

# Florence-2-large + bf16 on 16GB A4000: start at 4 images per GPU; if CUDA OOM, try 2 or 1.
PER_DEVICE_BATCH="${PER_DEVICE_BATCH:-2}"

# Optional: export TRAIN_PHASE=phase2 for honest training (loads phase1 checkpoint by default).
TRAIN_PHASE="${TRAIN_PHASE:-phase1}"

# Max COCO bbox instances to prepare (0 = full split, high RAM). Override as needed.
NUM_SAMPLES="${NUM_SAMPLES:-2000}"

accelerate launch \
    --num_processes=4 \
    --num_machines=2 \
    --machine_rank=0 \
    --main_process_ip="10.69.180.212" \
    --main_process_port=29500 \
    train.py \
    --phase "${TRAIN_PHASE}" \
    --coco_split train \
    --num_samples "${NUM_SAMPLES}" \
    --batch_size "${PER_DEVICE_BATCH}" \
    --epochs 10 \
    --output_dir ./checkpoints \
    --no_eval_baseline
